...
//! `dead_cst_runtime::register`. Keeping the `#[pymodule]` here (in a
//! cdylib) while the types live in `dead-cst-runtime` (an rlib/dylib) is
//! what lets an external native plugin dynamically link the *same* runtime
//! the extension module uses — one shared salsa/ty instance.
