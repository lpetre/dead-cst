...
2. **Graph materialization** (rust crate `dead_cst._native`). `Analysis.materialize_all()` instantiates a `native.ProjectContext` rooted at the project root (each package's path is passed as a `src_root`), registers each `native.NativePlugin`, and calls `materialize()`. The rust backend uses ty's `SemanticIndex` to:
...
3. **Plugin pass.** Each registered plugin is a `native.NativePlugin`. Project-wide plugins fan out across a GIL-free `rayon` scope inside `materialize()` (one `py.allow_threads` wrapping the scope, one worker per plugin), each running against a `FrozenView` — a `Send` snapshot of the build outputs — and pushing a `Vec<PreparedOp>` (`Node` / `Edge` / `Entrypoint` rows); per-file plugins are salsa-cached during the build. The rust apply pass folds every plugin's ops into the graph in one atomic batch, in registration order, before `materialize()` returns. After the pass returns, `Analysis` holds the live `ProjectContext` — there is no Python-side adjacency copy. `SymbolNode`, `Import`, `NodeFlags`, and `EdgeFlags` are re-exports of the rust pyclasses. The compiled rust extension lives at `python/dead_cst/_native.{abi3.so,pyd}` (built by maturin from `src/`) and is imported as `from dead_cst import _native as native`.
4. **Incremental rebuilds.** `Analysis.re_materialize(events)` applies the change events to the salsa db and re-runs the full materialize (skipping entirely when no salsa revision advanced). Everything global is refolded from the per-file parts on every build — nothing is patched in place, so nothing can be stale; the live-graph content (nodes + flags, edges, dead set) is exact against a fresh build, though dense ids differ once tombstones exist. The one retained layer is the cross-build resolution memo (`ResolveCache` in `runtime/src/project.rs`): each memoized cross-file resolution records its content read set, and a rebuild driven by content-only `Changed` events re-resolves only entries whose read set touches an effectively-changed file (event scope ∪ files whose salsa-tracked resolution-surface fingerprint moved — which is how `from X import *` importers propagate dirtiness without a hand-built dependency walk). `Created`/`Deleted`/`Rescan` or unknown paths invalidate the cache → full resolve. The builder output itself is per-file and persistent: each file owns a contiguous node block (O(1) `node_file`/`file_blocks` indirection) plus a translated out-edge part (`GraphBuilder::part_edges`); clean files' blocks and parts are reused verbatim, dirty files tombstone + re-append at the tail (live dense ids never remap; `ctx.tombstoned_indices()`), and a full build — taken automatically once tombstones outnumber live nodes — compacts. `ctx._last_resolve_counts()` reports `(resolved, reused)`.

5. **Reachability** (`Analysis.reachable` / `Analysis.dead` / `Analysis.descendants` / `Analysis.ancestors`). Bulk queries delegate to `ctx.reachable(...)` / `ctx.descendants(...)` / `ctx.ancestors(...)` — one FFI call per query, no per-node Python ↔ rust round-trips. Default traversal **does** follow `DEAD_BRANCH` edges (preserving today's behavior); `Analysis.kept_alive_by_dead_branches` is the opt-in inverse. `Analysis.kept_alive_by_flags_only(flags)` is the blast-radius query for any `NodeFlags` combination (`TESTCASE`, `NOQA`, or both). Path scoping is the caller's job: filter `analysis.dead()` by `Path(n.path).is_relative_to(pkg)` when you need a per-package slice.
...
Every built-in plugin is a native (Rust) `NativePlugin`, constructed via a factory: `NativePlugin.main_block()`, `NativePlugin.pytest()`, `NativePlugin.flask()` … `NativePlugin.celery()`, etc. There is no Python `Plugin` protocol. To add a built-in, write the impl in the runtime crate (`runtime/src/native_plugins.rs`) and wire its `--plugin` name into `_builtin_native_plugin`; the CLI's `_load_plugin` resolves names through that registry first, then the `dead_cst.plugins` entry-point group (whose target must resolve to, or return, a `NativePlugin`).

The plugin query surface lives on `native.ProjectContext` — the `*_indices` queries (`find_declarations_indices`, `module_surface_indices`, `decls_matching_indices`, `indices_where`, …) return positional indices into `ctx.nodes()`, materialized in bulk via `ctx.nodes_at(idxs)` / `ctx.node_attrs(idxs)` — plus direct accessors (`find_module_idx`, `module_for_indices`, `find_main_blocks_indices`, `find_factory_decls`, …). The decorator / construction / call walks the native plugins drive are rust-internal. See `python/dead_cst/_native.pyi` for the full surface.

To author an **out-of-tree native plugin** — compiled against the runtime `dylib` and loaded via `native.load_native_plugins(...)` — see [`NATIVE_PLUGINS.md`](NATIVE_PLUGINS.md). The author-facing flow is `pip install dead-cst[build-plugin]` (the shipped macOS/Linux wheel + the `dead-cst-plugin-host` rlib compile closure) then `dead-cst build-plugin <PLUGIN.rs>`, which compiles against the in-package runtime dylib + that closure — no swap. Maintainers populate the closure with `dead-cst bundle-plugin-host` (a prefer-dynamic build; the same build's runtime dylib is repacked into the base wheel). macOS + Linux only; needs the pinned toolchain. The `build-plugin`/`bundle-plugin-host` flow can't run from a dev/static checkout — it needs the dynamic wheel installed.
