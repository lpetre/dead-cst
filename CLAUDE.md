...
- **Cache invalidation discipline**: any change to a plugin / resolver / detector's *output* for the same input requires bumping its `version` to a fresh epoch, or stale cache entries will be served. The schema-level `SCHEMA_VERSION` in `_cache.py` is for `VisitorPayload` shape breaks the unpickler can't handle.
...
- Module-scope walrus expressions (PEP 572) bind names at runtime but aren't surfaced as top-level decls; the default detector also ignores walrus when constant-folding.
