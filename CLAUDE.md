...
4. **Bridge** (`python/dead_cst/_backend.py::_bridge`). Wrap the rust-shaped `NativeGraph` in a Python `SymbolGraph` — a thin dict-of-lists adjacency facade over the same `NativeNode` instances the rust pass produced. No type conversion: `SymbolNode` is a re-export of the rust `NativeNode` pyclass (so are `Import`, `NodeFlags`, `EdgeFlags`) and there's no parallel Python copy of the graph data. The `ProjectContext` stays alive past materialize so bulk traversal queries (`reachable`, `descendants`, `ancestors`) can be served by the rust BFS in one FFI hop. The compiled rust extension lives at `python/dead_cst/_native.{abi3.so,pyd}` (built by maturin from `src/`) and is imported as `from dead_cst import _native as native`.
5. **Reachability** (`Analysis.reachable` / `PackageView.reachable`). Bulk queries (`reachable` / `dead` / `descendants` / `ancestors` / `kept_alive_by_flags_only`) delegate to `ctx.reachable(...)` / `ctx.descendants(...)` / `ctx.ancestors(...)` — one FFI call per query, no per-node Python ↔ rust round-trips. Default traversal **does** follow `DEAD_BRANCH` edges (preserving today's behavior); `Analysis.kept_alive_by_dead_branches` is the opt-in inverse. `Analysis.kept_alive_by_flags_only(flags)` is the blast-radius query for any `NodeFlags` combination (`TESTCASE`, `NOQA`, or both).
