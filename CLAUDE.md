...
Four moving pieces — `SymbolVisitor` itself plus the three extension points (`EdgePlugin`, `PathResolver`, `UnreachableRegionDetector`) — all satisfy `Cacheable` (`dead_cst/_cacheable.py`), which is just `name: str` + `version: int`. The `(name, version)` pair tags each component, but only the visitor / plugin / detector triple feeds `compute_fingerprint` in `_cache.py` — swapping a `PathResolver` or bumping its `version` re-runs the (uncached) edge-stitching pass instead of invalidating the per-file SQLite cache, since resolution moved out of the visitor (stage 5 below). `version` is a Unix epoch int **by convention** — concurrent bumps on different branches merge with `max()`-wins semantics rather than colliding on a re-used label. Bump it any time a component's *output* would change for the same input. The package `__version__` is **not** in the fingerprint: each component carries its own knob, and folding `__version__` in on top would let lazily-unbumped components ride for free on a release bump. Schema version and Python version still participate.

### Pipeline (top-down)

1. **Path resolution** (`dead_cst/resolvers/`). A `PathResolver` returns a `PathMap` (`{base: [dep_paths]}`) plus an import resolver. Builtins: `VenvResolver` (sibling `.venv`), `PyprojectResolver` (`[tool.dead-cst]` paths or `src/` fallback), `UvWorkspaceResolver` (parses `uv.lock` to discover workspace members + inter-member dep edges), `ManualResolver`. Multiple resolvers chain via `_chain_resolvers` — first non-`None` wins. `resolvers/_imports.py` does name→path lookup; `resolvers/_exports.py` computes which subdirs are exposed to consumers.
...
8. **Codemod** (`dead_cst/codemod.py`). `remove_code` runs a LibCST `RemoveDeadSymbols` transformer keyed on `(fqname, CodeRange)` pairs (position disambiguates shadowed decls), then prunes now-unused imports via `RemoveImportsVisitor`. Position keying is critical — losing it conflates a dead decl with a live shadow. The high-level entry point is `PackageView.remove_dead_code()`, which materializes the closure, computes reachability, and feeds the unreachable subgraph for that base into `remove_code`.
...
- `dead_cst.resolvers` — the `PathResolver` protocol, generic builtin resolvers, and (re-exported) `UvWorkspaceResolver` from contrib.
- `dead_cst.contrib` — the canonical home for third-party-aware extensions: framework plugins (FastAPI, Flask, Click, Typer, pytest, unittest, mock-patch) and the `UvWorkspaceResolver`.
...
Implement `PathResolver`: `name`, `version`, `resolve(project_root) -> PathMap`, and `resolve_import(name, search_paths) -> str | Path | None`. Drop generic resolvers in `dead_cst/resolvers/<name>.py`; resolvers that target a specific external tool (like `UvWorkspaceResolver` for `uv.lock`) belong in `dead_cst/contrib/<name>.py`, with a re-export in `dead_cst/resolvers/__init__.py`. Third-party resolvers register under the `dead_cst.resolvers` entry-point group.
...
- **Cache invalidation discipline**: any change to the visitor / plugin / detector's *output* for the same input requires bumping its `version` to a fresh epoch, or stale cache entries will be served. Resolvers are *not* in the per-file fingerprint — their output flows through the (uncached) edge-stitching pass — so resolver `version` bumps don't invalidate cached payloads, but still serve as authoritative "I changed" markers for anything else that might key on them. `__version__` deliberately is not a backstop. The schema-level `SCHEMA_VERSION` in `_cache.py` is for `VisitorPayload` shape breaks the unpickler can't handle.
