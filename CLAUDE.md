...
- A `.py` whose stem matches a sibling regular package (`pkg/foo.py` next to `pkg/foo/__init__.py`) is shadowed: `_refresh.shadowed_paths` flags it before `_apply_payload`, and the apply pass sets `add_to_trie=False` so its decls never enter the trie. The file's nodes still land in the package graph (so observe-time entrypoints like `__main__` blocks keep working in the shadowed file), but consumer imports of `pkg.foo` route to the package's `__init__.py` alone — matching CPython's `FileFinder` precedence. A WARNING is logged per shadow. Notebooks (`.ipynb`) are explicitly skipped by this check — they aren't importable and so can never collide with a sibling package.
- `.ipynb` (Jupyter) files are ingested by `dead_cst/_notebooks.py`: code cells are concatenated in document order into a single libcst-parseable module, and IPython magics / shell escapes / trailing-help forms are neutralized line-for-line into `pass  # <line>` comments. `_refresh._process_one_file` passes `default_flags=NodeFlags.NOTEBOOK | NodeFlags.ENTRYPOINT` into the `SymbolVisitor`, which ORs it into every node it constructs — no separate payload-rewrite pass. The `NOTEBOOK` flag also keeps the decl out of the cross-module lookup trie (alongside `SHADOWED` / `OVERLOAD`). FQN derivation bypasses libcst's `gen_cache` (which has no notion of notebooks): `notebook_fqn_entry` sanitizes the stem to an identifier and synthesizes a `ModuleNameAndPackage` directly. `generate_patch` / `remove_code` skip any node flagged `NOTEBOOK` — cell-aware writeback into the notebook JSON envelope is out of scope today.
