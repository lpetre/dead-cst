...
1. **Path resolution** (`dead_cst/resolvers/`). A `PathResolver` returns `list[SourceTree]` plus an import resolver. Each `SourceTree` is a directory + `package` name + `SourceTreeFlags` + `search_trees` (paths of other trees this one's files can import from). At most one `EXPORTED` tree per package; only the exported tree's decls populate the package's consumer-visible export trie. Files are routed to their longest-prefix-matching tree, so a single package can split into `pkg/`, `pkg/tests/`, `pkg/scripts/` subtrees with different search paths. `validate_source_trees` enforces the invariants (one EXPORTED per package, every search ref points at an EXPORTED tree, no self-reference, acyclic). Builtins: `PyprojectResolver` (explicit `[tool.dead-cst].trees` or fallback to `src/` + sibling `tests/`), `UvWorkspaceResolver` (parses `uv.lock` to discover workspace members + inter-member dep edges), `ManualResolver` (`-p` CLI specs). Multiple resolvers chain via `_chain_resolvers` — first non-`None` wins. Run `dead-cst` with the project's venv active so `default_resolve_import` finds third-party dists via the running Python's `sys.path`. `resolvers/_imports.py` does name→path lookup.
...
4. **Visitor + observe cache** (`dead_cst/cache.py`). SQLite-backed `GraphCache` at `<root>/.dead-cst-cache/cache.db` stores pickled per-file payloads keyed by file SHA-256. Each row carries a per-tree fingerprint covering the tree path, its `search_trees`, and every visitor / resolver / plugin / detector `(name, version)` pair via `compute_fingerprint`. The cache's schema version lives in `meta`; a mismatch wipes `file_cache`. Each cache entry covers both the visitor's payload **and** every plugin's `observe()` output for that file — warm runs skip both. Bypass with `--no-cache`. Force-clear with `dead-cst cache clear`.
...
- `dead_cst.resolvers` — the `PathResolver` protocol, the `SourceTree` / `SourceTreeFlags` data types and `validate_source_trees` / `assign_file_to_tree` helpers, generic builtin resolvers, and (re-exported) `UvWorkspaceResolver` from contrib.
...
Implement `PathResolver`: `name`, `version`, `resolve(project_root) -> list[SourceTree]`, and `resolve_import(name, search_paths) -> str | Path | None`. Drop generic resolvers in `dead_cst/resolvers/<name>.py`; resolvers that target a specific external tool (like `UvWorkspaceResolver` for `uv.lock`) belong in `dead_cst/contrib/<name>.py`, with a re-export in `dead_cst/resolvers/__init__.py`. Third-party resolvers register under the `dead_cst.resolvers` entry-point group.
