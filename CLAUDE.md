...
3. **Plugin pass.** Each registered plugin is a `native.NativePlugin`. Project-wide plugins run against the in-progress `ProjectContext` via a parallel executor (`run_plugin_collect`) that collects each plugin's `AddNode` / `AddEdge` / `AddEntrypoint` ops into a handle; per-file plugins are salsa-cached during the build. The rust apply pass folds every handle into the graph in one atomic `apply_ops_batched` step. After the pass returns, `Analysis` holds the live `ProjectContext` — there is no Python-side adjacency copy. `SymbolNode`, `Import`, `NodeFlags`, and `EdgeFlags` are re-exports of the rust pyclasses. The compiled rust extension lives at `python/dead_cst/_native.{abi3.so,pyd}` (built by maturin from `src/`) and is imported as `from dead_cst import _native as native`.
