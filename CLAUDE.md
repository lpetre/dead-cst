...
3. **Plugin pass** has two phases: **(a) per-file plugins** run during the parallel populate phase, before the graph is assembled. Each `PerFilePlugin` implements `run_per_file(file: FileScope) -> Iterable[PerFileOp]` and gets a narrow file-local view (`FileScope.dunder_decls()`, `imports_of(module)`, `main_block()`, …) returning plain `int` indices into the file's `file_to_nodes` payload. Output is salsa-tracked keyed by `(file,)` via `file_to_plugin_ops` (`src/file_scope.rs`) — editing one file re-runs plugins only for that file. The assemble-time fan-in translates each file-local index to its global graph index via `ref_to_global`. **(b) Whole-project plugins** run after assemble: each `Plugin.run(ctx)` sees the in-progress `ProjectContext` and yields `AddNode` / `AddEdge` / `AddEntrypoint` ops (or their `*ByIdx` siblings); the rust apply pass folds them into the graph in one atomic step. After `materialize()` returns, `Analysis` holds the live `ProjectContext` — there is no Python-side adjacency copy. `SymbolNode`, `Import`, `NodeFlags`, and `EdgeFlags` are re-exports of the rust pyclasses. The compiled rust extension lives at `python/dead_cst/_native.{abi3.so,pyd}` (built by maturin from `src/`) and is imported as `from dead_cst import _native as native`.
...
**Per-file plugins** subclass `PerFilePlugin` and implement `run_per_file(file: native.FileScope) -> Iterable[PerFileOp]`. They run during populate, are salsa-cached per file, and can only reach file-local state (`FileScope` returns plain ints into the file's payload; `PerFileEdge` / `PerFileEntrypoint` / `PerFileNode` ops carry those ints; the fan-in translates them to global indices at assemble time). Migrate a plugin here whenever its output depends only on a single file's contents — `MainBlockPlugin` and `ModuleDundersPlugin` are the canonical examples.

**Whole-project plugins** subclass `Plugin` and follow the older shape:
