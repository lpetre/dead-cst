...
The plugin query surface lives on `native.ProjectContext` — `find_subclasses`, `find_module`, `find_declarations`, `module_for`, `find_main_blocks`, etc. — plus the chainable `query(ctx).decorators()...` / `.constructions()...` / `.calls()...` builder, usable from Python against a materialized context. See `python/dead_cst/_native.pyi` for the full surface.
