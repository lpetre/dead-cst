...
8. **Codemod** (`dead_cst/codemod.py`). `remove_code` runs a LibCST `RemoveDeadSymbols` transformer keyed on `(fqname, CodeRange)` pairs (position disambiguates shadowed decls), then prunes now-unused imports via `RemoveImportsVisitor`. Position keying is critical — losing it conflates a dead decl with a live shadow. The high-level entry point is `PackageView.remove_dead_code()`, which materializes the closure, computes reachability, and feeds the unreachable subgraph for that package into `remove_code`.
...
- `NodeFlags.SHADOWED` decls are emitted into the graph (with their parent module edge) but excluded from the trie, so cross-module imports never resolve to them. `NodeFlags.OVERLOAD` follows the same trie-exclusion rule but its lifetime is anchored to the matching impl via explicit `impl -> overload` edges (same-file via the visitor; cross-file via `PyiStubPlugin`).
- `.pyi` stub files participate as separate modules under a synthetic `__pyi__` FQN segment (stamped by `FixedFullyQualifiedNameProvider`) so `mod.py` and `mod.pyi` don't collide. `PyiStubPlugin` (a built-in, runs in `finalize`) wires `runtime_decl -> stub_decl` edges by simple-name match against the runtime FQN, so a stub's reachability tracks its `.py` twin and the codemod removes both together when the impl becomes dead.
