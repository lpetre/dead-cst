...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            id="lambda-conditional-expression-body",
        ),
        pytest.param(
            """
            A = 1
        ...
        pytest.param(
            ...
            id="lambda-default-argument-reference",
        ),
        pytest.param(
            """
            A = 1
        ...
        pytest.param(
            ...
            id="lambda-listcomp-body",
        ),
        pytest.param(
            """
            def helper(a, b): return a + b
            f = lambda x: lambda y: helper(x, y)
            """,
            # The inner lambda is a nested decl and gets no node of its
            # own; references inside it attribute to the enclosing
            # top-level decl ``mod.f``.
            {
                "mod.f -> mod",
                "mod.f -> mod.helper",
                "mod.helper -> mod",
            },
            id="lambda-nested-lambda-body",
        ),
        pytest.param(
            """
            N = "x"
