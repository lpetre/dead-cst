...
These tests are gated on the *plugin-host* build: ``scripts/build-plugin-host.sh``
builds the dynamic ``_native`` + the example plugin and runs them with
``DEAD_CST_PLUGIN_HOST=1`` / ``PLUGIN_DYLIB=<path>`` set. The default static
wheel build can't load external plugins (the runtime is statically linked,
not shared), so the load test skips there. The rejection test runs anywhere.
"""
...
@pytest.mark.skipif(
    not _PLUGIN_HOST,
    reason="external dylib plugins require the plugin-host build (scripts/build-plugin-host.sh)",
)
