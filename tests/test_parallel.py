...
def test_parallel_matches_serial(tmp_path):
    ...
    serial = Analysis(tmp_path, resolvers=manual()).materialize_all()
    parallel = Analysis(tmp_path, resolvers=manual(), workers=2).materialize_all()
...
def test_parallel_warms_cache(tmp_path):
    ...
    with GraphCache(db) as cache:
        cold = Analysis(tmp_path, resolvers=manual(), cache=cache, workers=2).materialize_all()
...
def test_parallel_falls_back_to_serial_for_single_miss(tmp_path, monkeypatch):
    ...
    with GraphCache(db) as cache:
        Analysis(tmp_path, resolvers=manual(), cache=cache, workers=4).materialize_all()
...
def test_parallel_pool_capped_at_total_task_count(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(analyze, "ProcessPoolExecutor", _spy)
    Analysis(tmp_path, resolvers=manual(), workers=64).materialize_all()
...
def test_workers_none_or_one_keeps_serial_path(tmp_path, monkeypatch, workers):
    ...
    monkeypatch.setattr(analyze, "ProcessPoolExecutor", _spy)
    Analysis(tmp_path, resolvers=manual(), workers=workers).materialize_all()
    assert calls == []
