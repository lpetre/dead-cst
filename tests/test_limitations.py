...
@pytest.mark.parametrize(
    ...
    [
        # ------------------------------------------------------------------
        # Top-level-only declaration tracking
        # ------------------------------------------------------------------
        pytest.param(
            {
                "mod.py": """
            ...
            # Ideally ``mod.b.c`` would be its own node with an edge to
            # ``mod.a``. Instead the reference is folded into ``mod.b``.
            {
                "mod.a -> mod",
                "mod.b -> mod",
                "mod.b -> mod.a",
            },
            id="nested-function-def-not-a-separate-node",
        ),
        pytest.param(
            {
                "mod.py": """
            ...
            # Ideally ``mod.Out.Inner`` would be a node.
            {
                "mod.Out -> mod",
            },
            id="nested-class-def-not-a-separate-node",
        ),
        pytest.param(
            {
                "mod.py": """
            ...
            # Ideally ``mod.f`` would point at the method ``mod.C.m``. For
            # now methods are not top-level decls, so the best we do is
            # link ``mod.f`` to the containing class.
            {
                "mod.C -> mod",
                "mod.f -> mod",
                "mod.f -> mod.C",
            },
            id="method-calls-link-to-class-not-method",
        ),
        pytest.param(
            {
                "mod.py": """
            ...
            # Ideally ``mod.B.m`` would point at ``mod.A.m``. Instead
            # only the class-level inheritance edge survives.
            {
                "mod.A -> mod",
                "mod.B -> mod",
                "mod.B -> mod.A",
            },
            id="super-call-has-no-direct-edge-to-overridden-method",
        ),
        # ------------------------------------------------------------------
