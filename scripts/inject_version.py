"""Stamp ``Cargo.toml`` with a setuptools-scm-style version.

Maturin reads the package version from ``Cargo.toml``'s ``[package].version``
field (``pyproject.toml`` is ``dynamic = ["version"]``). We don't hand-edit
that field; CI runs this script before each build to compute the version
from git history.
