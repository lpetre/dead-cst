...
- **`--query dead-tests` on `analyze` / `remove`, and `Analysis.dead_tests()`.**
  ...
  a test is *dead* when it exercises nothing a production entrypoint or a live
  test exercises. Liveness is a fixpoint -- live code is everything reachable
  from a non-test seed plus everything a live test references (transitively,
  through import aliases, fixtures, and the code they call), and a test is
  live when it references live code -- so the tests of pure test-support code
  (a custom assertion, a fake) whose only users are live tests are kept. The
  result is the blast radius of dropping those tests: each dead test plus
  every decl only dead tests keep alive, so `dead-cst remove --query dead-tests | git
