...
| `decorated_decls(fqnames)` | `Vec<usize>` | decls decorated by one of the decorators named by dotted `fqnames` (`"pytest.fixture"`), resolved like `find_subclasses_of_fqn` resolves a base class — a direct import, an alias, a re-export, a sibling-module spelling, `from X import *`, or a same-file definition all match; a target that doesn't resolve (dependency not installed, `pytest.mark.parametrize`) matches by its textual `module.name`; a builder suffix (`@launchable().cpus(16)`) is classified by its head call |
