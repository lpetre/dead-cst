...
| [`scripts-and-all/`](./scripts-and-all) | `ProjectScriptsPlugin`, `ModuleDundersPlugin`, `-p src` for the `src/` layout |
| [`uv-workspace/`](./uv-workspace) | `UvResolver` -- multi-package analysis driven by `uv.lock` |
...
# 2. project.scripts entrypoints + __all__ preservation, src/ layout
...
uv run dead-cst analyze examples/uv-workspace --resolver uv --plugin main_block
```
