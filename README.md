...
Delete the on-disk `VisitorPayload` cache (`<root>/.dead-cst-cache/`) for a project. The cache is keyed by a fingerprint over each (`Package`, phase) pair's `sys.path` entries and every `Cacheable` component (visitor, resolvers, plugins, unreachable-region detector), so most layout or analyzer-version changes invalidate it automatically; this command is for force-clearing when needed.
...
Path resolution is similarly pluggable. `PathResolver` implementations return a `list[Package]` to feed `Analysis`. Each `Package` is a directory `path` + unique `name` + `exported` subdirs (whose files ship in the wheel) + `deps` (other package names — the production-only DAG); files under `path` not under any `exported` subdir are *internal* (tests, scripts, app entrypoints). The analyzer parses each package in two phases — exported files in `deps` topo order, then internal files against the union of every package's export trie — so non-exported code can have apparent cross-package cycles (`A.tests` ↔ `B.lib`) without violating the production DAG. Builtins: `PyprojectResolver` (explicit `[[tool.dead-cst.packages]]` or fallback to a single package rooted at `project_root` with `exported` from the build backend), `UvResolver` (parses `uv.lock` to handle both single-package projects and workspaces with inter-member dep edges). Third-party resolvers register under `dead_cst.resolvers`. The `dead_cst.resolvers.exported_roots` and `exported_tree_root` helpers wrap the per-backend `pyproject.toml` parsing (hatchling / setuptools / poetry / pdm / flit) so custom resolvers can pick exported subdirs consistently. Run `dead-cst` with the project's venv active so `default_resolve_import` finds third-party dists via the running Python's `sys.path` -- `Analysis` warns at construction time if a sibling `.venv` / `venv` is found but isn't the running interpreter.
