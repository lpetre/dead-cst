...
`Analysis(...).materialize_all()` returns the full directed multigraph (a thin [rustworkx](https://www.rustworkx.org/)-backed wrapper exposing the networkx-style API the codebase uses) if you need raw access; `analysis.package(path).graph()` returns the closure-scoped subgraph for one package.
